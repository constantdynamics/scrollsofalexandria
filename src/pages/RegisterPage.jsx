...
const RegisterPage = () => {
  ...
  const categorieStats = useMemo(() => {
    ...
    principles.forEach(p => {
      const cat = p.academicCategory || 'Onbekend';
  ...
  const alleCategorieën = useMemo(() => categorieStats.map(c => c.naam), [categorieStats]);

  // Filter en sorteer principes
  const gefilterdePrincipes = useMemo(() => {
    let lijst = [...principles];

    if (geselecteerdeCategorie !== 'alle') {
      lijst = lijst.filter(p => p.academicCategory === geselecteerdeCategorie);
    }
    ...
    if (sorteerOp === 'titel') {
      ...
      lijst.sort((a, b) => (a.academicCategory || '').localeCompare(b.academicCategory || '', 'nl') || a.title.localeCompare(b.title, 'nl'));
    } else if (sorteerOp === 'moeilijkheid') {
      lijst.sort((a, b) => (a.difficulty || 1) - (b.difficulty || 1));
    }

    return lijst;
  }, [zoekterm, geselecteerdeCategorie, sorteerOp]);
  ...
  const getMasteryInfo = (principleId) => {
    ...
    return {
      ...
      unlocked: userData?.unlockedPrinciples?.includes(principleId) || false,
    };
  };

  const moeilijkheidLabel = (d) => {
    if (d === 1) return { label: 'Beginner', kleur: 'text-green-600 bg-green-50' };
    if (d === 2) return { label: 'Gevorderd', kleur: 'text-amber-600 bg-amber-50' };
    return { label: 'Expert', kleur: 'text-red-600 bg-red-50' };
  };

  const totaalBeheerst = principles.filter(p => {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-text-muted">{principles.length} principes · {categorieStats.length} categorieën</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="bg-primary-50 px-3 py-1.5 rounded-full border border-primary-100 text-xs font-medium text-primary">
                {totaalBeheerst}/{principles.length} beheerst
              </div>
            </div>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-bg-alt/50">
                    ...
                    <th className="text-right px-4 py-3 font-medium text-text-muted">Beheerst</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={beheerst === cat.count ? 'text-green-600 font-medium' : 'text-text-muted'}>
                            {beheerst}/{cat.count}
                          </span>
                        </td>
                      </tr>
                ...
                <tfoot>
                  <tr className="border-t-2 border-border bg-bg-alt/50">
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-text">{principles.length}</td>
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              ...
              <select
                ...
                className="px-3 py-2.5 rounded-xl border border-border bg-surface text-text focus:border-primary focus:outline-none text-sm min-w-[180px]"
              >
                <option value="alle">Alle categorieën ({principles.length})</option>
                {alleCategorieën.map(cat => (
              ...
              <select
                ...
                className="px-3 py-2.5 rounded-xl border border-border bg-surface text-text focus:border-primary focus:outline-none text-sm"
              >
                <option value="titel">Sorteren: A-Z</option>
                <option value="categorie">Sorteren: Categorie</option>
                <option value="moeilijkheid">Sorteren: Moeilijkheid</option>
              </select>
            </div>

            <p className="text-sm text-text-muted mb-3">
              {gefilterdePrincipes.length} van {principles.length} principes
              {geselecteerdeCategorie !== 'alle' && <span> in <strong className="text-text">{geselecteerdeCategorie}</strong></span>}
              {geselecteerdeCategorie !== 'alle' && (
                <button onClick={() => setGeselecteerdeCategorie('alle')} className="ml-2 text-primary underline text-xs">toon alles</button>
              )}
            </p>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {gefilterdePrincipes.map((p, index) => {
                      const { mastery, unlocked } = getMasteryInfo(p.id);
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-text-muted hidden md:table-cell text-xs">{p.academicCategory}</td>
